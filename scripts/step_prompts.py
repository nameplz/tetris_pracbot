...
from scripts.step_contracts import AgentRole, CIResult, ReviewFinding, sanitize_log


def implementation_prompt(*, phase: str, step: int, step_name: str, step_spec: str, feedback: str) -> str:
...
actual behavior, identify missing requirements, and do not edit files or commit. Directly
run unit tests and integration tests, then run Ruff, Mypy, and Pytest. Verify that tests
exercise externally observable behavior rather than implementation details. Report every
failure with its cause and a concrete recommendation for the implementation agent.
